...
def test_parse_github_full_name_supports_github_server_url_env(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_parse_github_full_name_rejects_unknown_hosts(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv('GITHUB_SERVER_URL', 'https://ghe.example.com')
    with pytest.raises(InvalidGitHubRemoteError):
        _parse_github_full_name('https://gitlab.com/org/repo.git')


def test_github_api_base_url_prefers_api_url(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_github_api_base_url_derives_from_server_url(monkeypatch: pytest.MonkeyPatch) -> None:
